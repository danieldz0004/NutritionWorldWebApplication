    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.SetPasswordSuccess ? "Your password has been set up."
                : message == ManageMessageId.SetTwoFactorSuccess ? "Your dual authentication provider has been set up."
        ...
        {
            ViewBag.UserModel = _dalUser.GetModel(User.Identity.GetUserId(), User.Identity.GetUserName());
            return View();
        }
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                userMod.Birthday = model.Birthday;
                userMod.FirstName = model.FirstName;
                userMod.LastName = model.LastName;
                _dalUser.Update(userMod);
                var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
                if (user != null)
                {
                    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
        ...
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.RemoveLoginSuccess ? "External login name has been deleted."
                : message == ManageMessageId.Error ? "Error."
                : "";
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user == null)
